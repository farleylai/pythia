...
def parse_args():
    ...
    parser.add_argument(
        '--meta', help='to save features with other image information', action='store_true'
    )
    parser.add_argument(
...
def extract_entities(path):
	csv = os.path.join(path, 'entities.csv')
	if os.path.exists(csv):
		return extract_bboxes(csv)

    if os.path.isdir(args.im_or_folder):
        annotations = glob.iglob(f'{path}/*.xml')

    image_bboxes = {}
    for i, annotation in enumerate(annotations):
		with open(annotation, "r") as xml:
			image_id = os.path.basename(annotation)[:-4]
			image_h, image_w = 0, 0
            ...
            for size in e.iter('size'):
                ...
                for s in size:
                	shape[s.tag] = s.text
                image_w = int(shape['width'])
                image_h = int(shape['height'])


			bbox_list = []
			for elem in e.iter('bndbox'):
				bbox_elem = {}
				for sub_elem in elem.iter():
					if sub_elem.tag != None:
						bbox_elem[sub_elem.tag] = sub_elem.text

				# if use roi_pooling, then there is no need to store each roi image. Just Store roi bbox ratios
				xmin_ratio = int(bbox_elem['xmin']) * 1.0 / image_size['width']
				xmax_ratio = int(bbox_elem['xmax']) * 1.0 / image_size['width']
				ymin_ratio = int(bbox_elem['ymin']) * 1.0 / image_size['height']
				ymax_ratio = int(bbox_elem['ymax']) * 1.0 / image_size['height']

				bbox_list.append(bbox_elem)
				bbox_list_ratio.append({'xmin_ratio':xmin_ratio,
...
def main(args):
    ...
    if args.bbox_file is not None:
        image_bboxes = extract_entities(args.bbox_file)
    ...
    for i, im_name in enumerate(im_list):
        ...
        if image_id % args.total_group == args.group_id:
            ...
            if im is not None:
                outfile = os.path.join(args.output_dir, im_base_name.replace('jpg', 'pth'))
                lock_folder = outfile.replace('pth', 'lock')
            ...
            if count % 100 == 0:
                ...
                print(f'Done image[{i}], processed {count} images after {epoch_time:.1f}s')
